...
func (ac *AssetController) PostAddAsset(c *gin.Context) {
	...
	if err := service.AssetServiceApp.AddAsset(global.DB, req, util.GetUUID(c)); err != nil {
		...
		common.ResponseOk(c, http.StatusInternalServerError, fmt.Sprintf("添加资产错误：%v", err.Error()), nil)
		return
	}
	common.ResponseOk(c, http.StatusOK, "添加资产成功", nil)
	return
}
...
func (ac *AssetController) PostDeleteAsset(c *gin.Context) {
	...
	if err != nil {
		...
		common.ResponseOk(c, http.StatusInternalServerError, fmt.Sprintf("删除资产失败: %v", err), nil)
		return
	}
	common.ResponseOk(c, http.StatusOK, "删除资产成功", nil)
	return
}
...
func (ac *AssetController) PostUpdateAsset(c *gin.Context) {
	...
	if err := service.AssetServiceApp.UpdateAsset(global.DB, req); err != nil {
		...
		common.ResponseOk(c, http.StatusInternalServerError, fmt.Sprintf("更新资产错误：%v", err.Error()), nil)
		return
	}
	common.ResponseOk(c, http.StatusOK, "更新资产成功", nil)
	return
}
...
func (ac *AssetController) PostFetchAssetIpDetail(c *gin.Context) {
	common.ResponseOk(c, http.StatusOK, "获取资产 IP 详情", nil)
	return
}

func (ac *AssetController) PostFetchAssetDomainDetail(c *gin.Context) {
	common.ResponseOk(c, http.StatusOK, "获取资产域名详情", nil)
	return
}
